...
const Step5Summary = ({ formData }) => {
  ...
  const { totalFixed, totalVariable, totalSavings, totalOutflow, remaining } = useMemo(() => {
  ...
  const chartData = simulateMode
    ? [
        ...
        { name: "Remaining", value: Math.max(0, (formData?.salary || 0) - (simulatedValues.fixed + simulatedValues.variable + simulatedValues.savings)) },
      ]
    : [
...
<div className="summary-content two-column-layout">
  ...
  <div className="right-column">
    ...
    <SummaryFooter
      ...
      remaining={
        simulateMode
          ? Math.max(0, (formData?.salary || 0) - (simulatedValues.fixed + simulatedValues.variable + simulatedValues.savings))
          : remaining
      }
    />
  </div>
